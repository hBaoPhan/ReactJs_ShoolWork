import { r as __toESM, t as require_react } from "./react-DV3nHG90.js";
import { t as require_react_dom } from "./react-dom-BjDyeKcB.js";
//#region node_modules/recoil/es/index.js
		...
		return loadableWithPromise(this.contents.then((value) => {
			...
			if (isLoadable(next)) {
				...
				switch (nextLoadable.state) {
					case "hasValue": return nextLoadable.contents;
					case "hasError": throw nextLoadable.contents;
					case "loading": return nextLoadable.contents;
				}
			}
			return next;
		}).catch((e) => {
			if (Recoil_isPromise(e)) return e.then(() => this.map(map).contents);
			throw e;
		}));
	}
};
...
function loadableAll(inputs) {
	const output = loadableAllArray((Array.isArray(inputs) ? inputs : Object.getOwnPropertyNames(inputs).map((key) => inputs[key])).map((x) => isLoadable(x) ? x : Recoil_isPromise(x) ? loadableWithPromise(x) : loadableWithValue(x)));
...
function readProcessEnvBooleanFlag(name, set) {
	...
	if (!["true", "false"].includes(sanitizedValue)) throw Recoil_err(`process.env.${name} value must be 'true', 'false', or empty: ${sanitizedValue}`);
...
var useSyncExternalStore = (_useSyncExternalStore = import_react.useSyncExternalStore) !== null && _useSyncExternalStore !== void 0 ? _useSyncExternalStore : import_react.default.unstable_useSyncExternalStore;
...
function currentRendererSupportsUseSyncExternalStore() {
	...
	const { ReactCurrentDispatcher, ReactCurrentOwner } = import_react.__SECRET_INTERNALS_DO_NOT_USE_OR_YOU_WILL_BE_FIRED;
	const isUseSyncExternalStoreSupported = ((_ReactCurrentDispatch = ReactCurrentDispatcher === null || ReactCurrentDispatcher === void 0 ? void 0 : ReactCurrentDispatcher.current) !== null && _ReactCurrentDispatch !== void 0 ? _ReactCurrentDispatch : ReactCurrentOwner.currentDispatcher).useSyncExternalStore != null;
...
var { isFastRefreshEnabled: isFastRefreshEnabled$1 } = Recoil_ReactMode;
...
function checkForDuplicateAtomKey(key) {
	if (nodes.has(key)) {
      ...
      hot module replacement.`;
		if (!isFastRefreshEnabled$1()) Recoil_expectationViolation(message, "recoil");
	}
}
...
function setConfigDeletionHandler(key, fn) {
	if (!Recoil_gkx("recoil_memory_managament_2020")) return;
	if (fn === void 0) configDeletionHandlers.delete(key);
...
var Recoil_Queue = { enqueueExecution };
...
var hamt_1 = createCommonjsModule(function(module) {
	...
	*/
	...
	var Collision__modify = function Collision__modify(edit, keyEq, shift, f, h, k, size) {
		if (h === this.hash) {
			var list = updateCollisionList(canEditNode(edit, this), edit, keyEq, this.hash, this.children, f, k, size);
	...
	*/
	var tryGetHash = hamt.tryGetHash = function(alt, hash, key, map) {
		...
		while (true) switch (node.type) {
			case LEAF: return keyEq(key, node.key) ? node.value : alt;
			case COLLISION:
				if (hash === node.hash) {
				...
				return alt;
			case INDEX:
				var bit = toBitmap(hashFragment(shift, hash));
				if (node.mask & bit) {
	...
	*/
	var getHash = hamt.getHash = function(hash, key, map) {
		return tryGetHash(void 0, hash, key, map);
	};
	...
	*/
	var fold = hamt.fold = function(f, z, m) {
		...
		while (children = toVisit.pop()) for (var i = 0, len = children.length; i < len;) {
			var child = children[i++];
			if (child && child.type) if (child.type === LEAF) z = f(z, child.value, child.key);
...
var HashArrayMappedTrieMap = class HashArrayMappedTrieMap {
	constructor(existing) {
...
var Recoil_PersistentMap_1 = { persistentMap }.persistentMap;
...
function mergeDepsIntoGraph(key, newDeps, graph, olderGraph) {
	...
	for (const dep of addedDeps) {
		if (!nodeToNodeSubscriptions.has(dep)) nodeToNodeSubscriptions.set(dep, /* @__PURE__ */ new Set());
		Recoil_nullthrows(nodeToNodeSubscriptions.get(dep)).add(key);
	}
	if (oldDeps) {
...
function saveDepsToStore(key, deps, store, version) {
	...
	if (version === ((_storeState$previousT2 = storeState.previousTree) === null || _storeState$previousT2 === void 0 ? void 0 : _storeState$previousT2.version)) mergeDepsIntoGraph(key, deps, store.getGraph(storeState.currentTree.version), graph);
	if (version === ((_storeState$previousT3 = storeState.previousTree) === null || _storeState$previousT3 === void 0 ? void 0 : _storeState$previousT3.version) || version === storeState.currentTree.version) {
		var _storeState$nextTree2;
		const nextVersion = (_storeState$nextTree2 = storeState.nextTree) === null || _storeState$nextTree2 === void 0 ? void 0 : _storeState$nextTree2.version;
		if (nextVersion !== void 0) mergeDepsIntoGraph(key, deps, store.getGraph(nextVersion), graph);
	}
}
var Recoil_Graph = {
	cloneGraph,
...
var { persistentMap: persistentMap$1 } = Recoil_PersistentMap$1;
var { graph } = Recoil_Graph;
var { getNextTreeStateVersion: getNextTreeStateVersion$1 } = Recoil_Keys;
...
var { getNode: getNode$1, getNodeMaybe: getNodeMaybe$1, recoilValuesForKeys: recoilValuesForKeys$1 } = Recoil_Node;
var { RetentionZone: RetentionZone$1 } = Recoil_RetentionZone;
var { setByAddingToSet: setByAddingToSet$1 } = Recoil_CopyOnWrite;
var emptySet = Object.freeze(/* @__PURE__ */ new Set());
var ReadOnlyRecoilValueError = class extends Error {};
function initializeRetentionForNode(store, nodeKey, retainedBy) {
	if (!Recoil_gkx("recoil_memory_managament_2020")) return () => void 0;
	const { nodesRetainedByZone } = store.getState().retention;
	function addToZone(zone) {
		let set = nodesRetainedByZone.get(zone);
		if (!set) nodesRetainedByZone.set(zone, set = /* @__PURE__ */ new Set());
		set.add(nodeKey);
	}
...
function setUnvalidatedAtomValue_DEPRECATED(state, key, newValue) {
	...
	node === null || node === void 0 || (_node$invalidate = node.invalidate) == null || _node$invalidate.call(node, state);
	return {
		...state,
	...
	return Recoil_lazyProxy({ type }, {
		...
		subscribers: () => {
			...
			return {
				...
				components: Recoil_mapIterable((_storeState$nodeToCom = (_storeState$nodeToCom2 = storeState.nodeToComponentSubscriptions.get(key)) === null || _storeState$nodeToCom2 === void 0 ? void 0 : _storeState$nodeToCom2.values()) !== null && _storeState$nodeToCom !== void 0 ? _storeState$nodeToCom : [], ([name]) => ({ name }))
			};
		}
	});
}
...
var { getDownstreamNodes: getDownstreamNodes$1, getNodeLoadable: getNodeLoadable$1, setNodeValue: setNodeValue$1 } = Recoil_FunctionalCore;
var { getNextComponentID: getNextComponentID$1 } = Recoil_Keys;
var { getNode: getNode$2, getNodeMaybe: getNodeMaybe$2 } = Recoil_Node;
var { DefaultValue: DefaultValue$1 } = Recoil_Node;
var { reactMode: reactMode$1 } = Recoil_ReactMode;
var { AbstractRecoilValue: AbstractRecoilValue$1, RecoilState: RecoilState$1, RecoilValueReadOnly: RecoilValueReadOnly$1, isRecoilValue: isRecoilValue$1 } = Recoil_RecoilValue$1;
var { invalidateMemoizedSnapshot: invalidateMemoizedSnapshot$1 } = Recoil_SnapshotCache;
...
function queueOrPerformStateUpdate(store, action) {
	if (batchStack.length) {
		...
		actions.push(action);
	} else applyActionsToStore(store, [action]);
}
var batchStack = [];
function batchStart() {
	...
	return () => {
		...
		if (batchStack.pop() !== actionsByStore) Recoil_recoverableViolation("Incorrect order of batch popping");
	};
}
...
function invalidateDownstreams(store, state) {
	...
	for (const key of downstreams) {
		...
		(_getNodeMaybe = getNodeMaybe$2(key)) === null || _getNodeMaybe === void 0 || (_getNodeMaybe$invalid = _getNodeMaybe.invalidate) == null || _getNodeMaybe$invalid.call(_getNodeMaybe, state);
	}
}
...
function subscribeToRecoilValue(store, { key }, callback, componentDebugName = null) {
	...
	const storeState = store.getState();
	if (!storeState.nodeToComponentSubscriptions.has(key)) storeState.nodeToComponentSubscriptions.set(key, /* @__PURE__ */ new Map());
...
var { cleanUpNode: cleanUpNode$1 } = Recoil_FunctionalCore;
var { deleteNodeConfigIfPossible: deleteNodeConfigIfPossible$1, getNode: getNode$3 } = Recoil_Node;
var { RetentionZone: RetentionZone$2 } = Recoil_RetentionZone;
...
function releaseRetainablesNowOnCurrentTree(store, retainables) {
	...
	for (const r of retainables) if (r instanceof RetentionZone$2) for (const n of nodesRetainedByZone(storeState, r)) nodes.add(n);
	else nodes.add(r);
...
function findReleasableNodes(store, searchFromNodes) {
	...
	function findReleasableNodesInner(searchFromNodes) {
		...
		const downstreams = getDownstreamNodesInTopologicalOrder(store, treeState, searchFromNodes, releasableNodes, nonReleasableNodes);
...
function zonesThatCouldRetainNode(node) {
	...
	if (retainedBy === void 0 || retainedBy === "components" || retainedBy === "recoilRoot") return [];
	else if (retainedBy instanceof RetentionZone$2) return [retainedBy];
	else return retainedBy;
}
function scheduleOrPerformPossibleReleaseOfRetainable(store, retainable) {
	const state = store.getState();
	if (state.nextTree) state.retention.retainablesToCheckForRelease.add(retainable);
	else releaseRetainablesNowOnCurrentTree(store, new Set([retainable]));
}
...
function updateRetainCountToZero(store, retainable) {
	if (!Recoil_gkx("recoil_memory_managament_2020")) return;
	store.getState().retention.referenceCounts.delete(retainable);
...
* @oncall recoil
*/
var { unstable_batchedUpdates } = import_react_dom.default;
/**
* Copyright (c) Meta Platforms, Inc. and affiliates.
*
* This source code is licensed under the MIT license found in the
...
* @oncall recoil
*/
var { unstable_batchedUpdates: unstable_batchedUpdates$1 } = { unstable_batchedUpdates };
...
* @oncall recoil
*/
var { batchStart: batchStart$1 } = Recoil_RecoilValueInterface;
var { unstable_batchedUpdates: unstable_batchedUpdates$2 } = Recoil_ReactBatchedUpdates;
...
var { batchUpdates: batchUpdates$1 } = Recoil_Batching;
var { initializeNode: initializeNode$1, peekNodeInfo: peekNodeInfo$1 } = Recoil_FunctionalCore;
var { graph: graph$1 } = Recoil_Graph;
var { getNextStoreID: getNextStoreID$1 } = Recoil_Keys;
var { DEFAULT_VALUE: DEFAULT_VALUE$1, recoilValues: recoilValues$1, recoilValuesForKeys: recoilValuesForKeys$2 } = Recoil_Node;
var { AbstractRecoilValue: AbstractRecoilValue$2, getRecoilValueAsLoadable: getRecoilValueAsLoadable$1, setRecoilValue: setRecoilValue$1, setUnvalidatedRecoilValue: setUnvalidatedRecoilValue$1 } = Recoil_RecoilValueInterface;
var { updateRetainCount: updateRetainCount$1 } = Recoil_Retention;
var { setInvalidateMemoizedSnapshot: setInvalidateMemoizedSnapshot$1 } = Recoil_SnapshotCache;
var { getNextTreeStateVersion: getNextTreeStateVersion$2, makeEmptyStoreState: makeEmptyStoreState$1 } = Recoil_State;
var { isSSR: isSSR$1 } = Recoil_Environment;
var { memoizeOneWithArgsHashAndInvalidation: memoizeOneWithArgsHashAndInvalidation$1 } = Recoil_Memoize;
var retainWarning = `
...
var Snapshot = class {
	constructor(storeState, parentStoreID) {
		...
		_defineProperty(this, "getNodes_UNSTABLE", (opt) => {
			...
			return (opt === null || opt === void 0 ? void 0 : opt.isInitialized) == null ? recoilValues$1.values() : opt.isInitialized === true ? recoilValuesForKeys$2(Recoil_concatIterables([knownAtoms, knownSelectors])) : Recoil_filterIterable(recoilValues$1.values(), ({ key }) => !knownAtoms.has(key) && !knownSelectors.has(key));
		});
	...
	retain() {
		if (this._refCount <= 0) throw Recoil_err("Snapshot has already been released.");
		this._refCount++;
		let released = false;
		return () => {
			if (!released) {
	...
	*/
	autoRelease_INTERNAL() {
		if (!isSSR$1) window.setTimeout(() => this._release(), 10);
	}
	_release() {
		this._refCount--;
		if (this._refCount === 0) {
			...
			if (!Recoil_gkx("recoil_memory_managament_2020")) return;
		} else if (this._refCount < 0) Recoil_recoverableViolation("Snapshot released an extra time.");
	}
	isRetained() {
		return this._refCount > 0;
	}
	checkRefCount_INTERNAL() {
		if (Recoil_gkx("recoil_memory_managament_2020") && this._refCount <= 0) Recoil_recoverableViolation(retainWarning);
	}
	getStore_INTERNAL() {
		this.checkRefCount_INTERNAL();
		return this._store;
...
var [memoizedCloneSnapshot, invalidateMemoizedSnapshot$2] = memoizeOneWithArgsHashAndInvalidation$1((store, version) => {
	...
	const storeState = store.getState();
	return new Snapshot(cloneStoreState(store, version === "latest" ? (_storeState$nextTree = storeState.nextTree) !== null && _storeState$nextTree !== void 0 ? _storeState$nextTree : storeState.currentTree : Recoil_nullthrows(storeState.previousTree)), store.storeID);
}, (store, version) => {
...
function useRefInitOnce(initialValue) {
	...
	if (ref.current === initialValue && typeof initialValue === "function") ref.current = initialValue();
	return ref;
}
var Recoil_useRefInitOnce = useRefInitOnce;
var { getNextTreeStateVersion: getNextTreeStateVersion$3, makeEmptyStoreState: makeEmptyStoreState$2 } = Recoil_State;
var { cleanUpNode: cleanUpNode$2, getDownstreamNodes: getDownstreamNodes$2, initializeNode: initializeNode$2, setNodeValue: setNodeValue$2, setUnvalidatedAtomValue_DEPRECATED: setUnvalidatedAtomValue_DEPRECATED$1 } = Recoil_FunctionalCore;
var { graph: graph$2 } = Recoil_Graph;
var { cloneGraph: cloneGraph$1 } = Recoil_Graph;
var { getNextStoreID: getNextStoreID$2 } = Recoil_Keys;
var { createMutableSource: createMutableSource$1, reactMode: reactMode$2 } = Recoil_ReactMode;
...
var { useCallback, useContext, useEffect, useMemo, useRef: useRef$1, useState } = import_react.default;
...
var AppContext = import_react.createContext({ current: defaultStore });
var useStoreRef = () => useContext(AppContext);
var MutableSourceContext = import_react.createContext(null);
function useRecoilMutableSource() {
	const mutableSource = useContext(MutableSourceContext);
	if (mutableSource == null) Recoil_expectationViolation("Attempted to use a Recoil hook outside of a <RecoilRoot>. <RecoilRoot> must be an ancestor of any component that uses Recoil hooks.");
...
function sendEndOfBatchNotifications(store) {
	...
	if (dirtyAtoms.size) {
		for (const [key, subscriptions] of storeState.nodeTransactionSubscriptions) if (dirtyAtoms.has(key)) for (const [_, subscription] of subscriptions) subscription(store);
...
if (typeof window !== "undefined" && !window.$recoilDebugStates) window.$recoilDebugStates = [];
...
var nextID = 0;
function RecoilRoot_INTERNAL({ initializeState_DEPRECATED, initializeState, store_INTERNAL: storeProp, children }) {
	let storeStateRef;
	...
	const subscribeToTransactions = (callback, key) => {
		if (key == null) {
			...
			if (!nodeTransactionSubscriptions.has(key)) nodeTransactionSubscriptions.set(key, /* @__PURE__ */ new Map());
			const id = nextID++;
	...
	const replaceState = (replacer) => {
		...
		if (typeof window !== "undefined") window.$recoilDebugStates.push(replaced);
	...
	return /* @__PURE__ */ import_react.createElement(AppContext.Provider, { value: storeRef }, /* @__PURE__ */ import_react.createElement(MutableSourceContext.Provider, { value: mutableSource }, /* @__PURE__ */ import_react.createElement(Batcher, { setNotifyBatcherOfChange }), children));
}
function RecoilRoot(props) {
	const { override, ...propsExceptOverride } = props;
	const ancestorStoreRef = useStoreRef();
	if (override === false && ancestorStoreRef.current !== defaultStore) return props.children;
	return /* @__PURE__ */ import_react.createElement(RecoilRoot_INTERNAL, propsExceptOverride);
}
...
var { useEffect: useEffect$1, useRef: useRef$2 } = import_react.default;
...
var { useStoreRef: useStoreRef$1 } = Recoil_RecoilRoot;
var { SUSPENSE_TIMEOUT_MS: SUSPENSE_TIMEOUT_MS$1 } = Recoil_Retention;
var { updateRetainCount: updateRetainCount$2 } = Recoil_Retention;
var { RetentionZone: RetentionZone$3 } = Recoil_RetentionZone;
var { useEffect: useEffect$2, useRef: useRef$3 } = import_react.default;
var { isSSR: isSSR$2 } = Recoil_Environment;
function useRetain(toRetain) {
	if (!Recoil_gkx("recoil_memory_managament_2020")) return;
	return useRetain_ACTUAL(toRetain);
}
function useRetain_ACTUAL(toRetain) {
	const retainables = (Array.isArray(toRetain) ? toRetain : [toRetain]).map((a) => a instanceof RetentionZone$3 ? a : a.key);
...
var { batchUpdates: batchUpdates$2 } = Recoil_Batching;
var { DEFAULT_VALUE: DEFAULT_VALUE$2 } = Recoil_Node;
var { currentRendererSupportsUseSyncExternalStore: currentRendererSupportsUseSyncExternalStore$1, reactMode: reactMode$3, useMutableSource: useMutableSource$1, useSyncExternalStore: useSyncExternalStore$1 } = Recoil_ReactMode;
var { useRecoilMutableSource: useRecoilMutableSource$1, useStoreRef: useStoreRef$2 } = Recoil_RecoilRoot;
var { isRecoilValue: isRecoilValue$2 } = Recoil_RecoilValue$1;
var { AbstractRecoilValue: AbstractRecoilValue$3, getRecoilValueAsLoadable: getRecoilValueAsLoadable$2, setRecoilValue: setRecoilValue$2, setUnvalidatedRecoilValue: setUnvalidatedRecoilValue$2, subscribeToRecoilValue: subscribeToRecoilValue$1 } = Recoil_RecoilValueInterface;
var { useCallback: useCallback$1, useEffect: useEffect$3, useMemo: useMemo$1, useRef: useRef$4, useState: useState$1 } = import_react.default;
var { setByAddingToSet: setByAddingToSet$2 } = Recoil_CopyOnWrite;
var { isSSR: isSSR$3 } = Recoil_Environment;
function handleLoadable(loadable, recoilValue, storeRef) {
	if (loadable.state === "hasValue") return loadable.contents;
	else if (loadable.state === "loading") throw new Promise((resolve) => {
...
function useRecoilInterface_DEPRECATED() {
	...
	useEffect$3(() => {
		...
		Recoil_differenceSets(recoilValuesUsed.current, previousSubscriptions.current).forEach((key) => {
			...
			if (store.getState().nextTree) store.getState().queuedComponentCallbacks_DEPRECATED.push(() => {
	...
	return useMemo$1(() => {
		...
		function useRecoilValue(recoilValue) {
			...
			return handleLoadable(useRecoilValueLoadable(recoilValue), recoilValue, storeRef);
		}
...
var recoilComponentGetRecoilValueCount_FOR_TESTING = { current: 0 };
function useRecoilValueLoadable_SYNC_EXTERNAL_STORE(recoilValue) {
	...
	const getSnapshot = useCallback$1(() => {
		...
		const storeState = store.getState();
		return {
			loadable: getRecoilValueAsLoadable$2(store, recoilValue, reactMode$3().early ? (_storeState$nextTree2 = storeState.nextTree) !== null && _storeState$nextTree2 !== void 0 ? _storeState$nextTree2 : storeState.currentTree : storeState.currentTree),
			key: recoilValue.key
		};
	...
	return useSyncExternalStore$1(useCallback$1((notify) => {
		const store = storeRef.current;
		return subscribeToRecoilValue$1(store, recoilValue, notify, componentName).release;
	}, [
		storeRef,
		recoilValue,
		componentName
	]), getMemoizedSnapshot, getMemoizedSnapshot).loadable;
}
function useRecoilValueLoadable_MUTABLE_SOURCE(recoilValue) {
	...
	const getLoadable = useCallback$1(() => {
		...
		const store = storeRef.current;
		const storeState = store.getState();
		return getRecoilValueAsLoadable$2(store, recoilValue, reactMode$3().early ? (_storeState$nextTree3 = storeState.nextTree) !== null && _storeState$nextTree3 !== void 0 ? _storeState$nextTree3 : storeState.currentTree : storeState.currentTree);
	}, [storeRef, recoilValue]);
	...
	const subscribe = useCallback$1((_storeState, notify) => {
		const store = storeRef.current;
		return subscribeToRecoilValue$1(store, recoilValue, () => {
	...
	if (source == null) throw Recoil_err("Recoil hooks must be used in components contained within a <RecoilRoot> component.");
...
function useRecoilValueLoadable_TRANSITION_SUPPORT(recoilValue) {
	...
	const getLoadable = useCallback$1(() => {
		...
		const store = storeRef.current;
		const storeState = store.getState();
		return getRecoilValueAsLoadable$2(store, recoilValue, reactMode$3().early ? (_storeState$nextTree4 = storeState.nextTree) !== null && _storeState$nextTree4 !== void 0 ? _storeState$nextTree4 : storeState.currentTree : storeState.currentTree);
	}, [storeRef, recoilValue]);
...
function useRecoilValueLoadable_LEGACY(recoilValue) {
	...
	const getLoadable = useCallback$1(() => {
		...
		const store = storeRef.current;
		const storeState = store.getState();
		return getRecoilValueAsLoadable$2(store, recoilValue, reactMode$3().early ? (_storeState$nextTree5 = storeState.nextTree) !== null && _storeState$nextTree5 !== void 0 ? _storeState$nextTree5 : storeState.currentTree : storeState.currentTree);
	}, [storeRef, recoilValue]);
	...
	useEffect$3(() => {
		...
		const subscription = subscribeToRecoilValue$1(store, recoilValue, (_state) => {
			...
			const newLoadable = getLoadable();
			if (!((_prevLoadableRef$curr = prevLoadableRef.current) !== null && _prevLoadableRef$curr !== void 0 && _prevLoadableRef$curr.is(newLoadable))) forceUpdate(newLoadable);
			prevLoadableRef.current = newLoadable;
		}, componentName);
		/**
		...
		*/
		if (storeState.nextTree) store.getState().queuedComponentCallbacks_DEPRECATED.push(() => {
			...
			if (!Recoil_gkx("recoil_suppress_rerender_in_callback")) return forceUpdate([]);
			const newLoadable = getLoadable();
			if (!((_prevLoadableRef$curr2 = prevLoadableRef.current) !== null && _prevLoadableRef$curr2 !== void 0 && _prevLoadableRef$curr2.is(newLoadable))) forceUpdate(newLoadable);
			prevLoadableRef.current = newLoadable;
		}
		return subscription.release;
	}, [
...
function useRecoilValueLoadable(recoilValue) {
	validateRecoilValue(recoilValue, "useRecoilValueLoadable");
	if (Recoil_gkx("recoil_memory_managament_2020")) Recoil_useRetain(recoilValue);
	return {
...
function useRecoilValueLoadable_TRANSITION_SUPPORT_UNSTABLE(recoilValue) {
	...
	if (!reactMode$3().early) Recoil_recoverableViolation("Attepmt to use a hook with UNSTABLE_TRANSITION_SUPPORT in a rendering mode incompatible with concurrent rendering.  Try enabling the recoil_sync_external_store or recoil_transition_support GKs.");
	if (Recoil_gkx("recoil_memory_managament_2020")) Recoil_useRetain(recoilValue);
...
function useRecoilValue_TRANSITION_SUPPORT_UNSTABLE(recoilValue) {
	...
	const storeRef = useStoreRef$2();
	return handleLoadable(useRecoilValueLoadable_TRANSITION_SUPPORT_UNSTABLE(recoilValue), recoilValue, storeRef);
}
...
function mergeMaps(...maps) {
	...
	for (let i = 0; i < maps.length; i++) {
		...
		let nextKey;
		while (!(nextKey = iterator.next()).done) result.set(nextKey.value, maps[i].get(nextKey.value));
	}
	return result;
}
var Recoil_mergeMaps = mergeMaps;
var { batchUpdates: batchUpdates$3 } = Recoil_Batching;
var { DEFAULT_VALUE: DEFAULT_VALUE$3, getNode: getNode$4, nodes: nodes$1 } = Recoil_Node;
var { useStoreRef: useStoreRef$3 } = Recoil_RecoilRoot;
var { AbstractRecoilValue: AbstractRecoilValue$4, setRecoilValueLoadable: setRecoilValueLoadable$1 } = Recoil_RecoilValueInterface;
var { SUSPENSE_TIMEOUT_MS: SUSPENSE_TIMEOUT_MS$2 } = Recoil_Retention;
var { cloneSnapshot: cloneSnapshot$1 } = Recoil_Snapshot$1;
var { useCallback: useCallback$2, useEffect: useEffect$4, useRef: useRef$5, useState: useState$2 } = import_react.default;
var { isSSR: isSSR$4 } = Recoil_Environment;
...
function externallyVisibleAtomValuesInState(state) {
	const persistedAtomContentsValues = Recoil_mapMap(Recoil_filterMap(state.atomValues.toMap(), (v, k) => {
		const persistence = getNode$4(k).persistence_UNSTABLE;
...
function useTransactionObservation_DEPRECATED(callback) {
	useTransactionSubscription(useCallback$2((store) => {
		...
		callback({
			...
			modifiedAtoms: Recoil_filterSet(currentTree.dirtyAtoms, (k) => atomValues.has(k) || previousAtomValues.has(k)),
...
function useRecoilTransactionObserver(callback) {
	useTransactionSubscription(useCallback$2((store) => {
		callback({
			snapshot: cloneSnapshot$1(store, "latest"),
...
function gotoSnapshot(store, snapshot) {
	...
	batchUpdates$3(() => {
		...
		for (const keys of [prev.atomValues.keys(), next.atomValues.keys()]) for (const key of keys) {
...
var { peekNodeInfo: peekNodeInfo$2 } = Recoil_FunctionalCore;
var { useStoreRef: useStoreRef$4 } = Recoil_RecoilRoot;
...
var { reactMode: reactMode$4 } = Recoil_ReactMode;
var { RecoilRoot: RecoilRoot$1, useStoreRef: useStoreRef$5 } = Recoil_RecoilRoot;
var { useMemo: useMemo$2 } = import_react.default;
function useRecoilBridgeAcrossReactRoots() {
	if (reactMode$4().mode === "MUTABLE_SOURCE") console.warn("Warning: There are known issues using useRecoilBridgeAcrossReactRoots() in recoil_mutable_source rendering mode.  Please consider upgrading to recoil_sync_external_store mode.");
	const store = useStoreRef$5().current;
	return useMemo$2(() => {
		function RecoilBridge({ children }) {
			return /* @__PURE__ */ import_react.createElement(RecoilRoot$1, { store_INTERNAL: store }, children);
		}
		return RecoilBridge;
	}, [store]);
}
var Recoil_useRecoilBridgeAcrossReactRoots = useRecoilBridgeAcrossReactRoots;
var { loadableWithValue: loadableWithValue$1 } = Recoil_Loadable$1;
var { initializeNode: initializeNode$3 } = Recoil_FunctionalCore;
var { DEFAULT_VALUE: DEFAULT_VALUE$4, getNode: getNode$5 } = Recoil_Node;
var { copyTreeState: copyTreeState$1, getRecoilValueAsLoadable: getRecoilValueAsLoadable$3, invalidateDownstreams: invalidateDownstreams$1, writeLoadableToTreeState: writeLoadableToTreeState$1 } = Recoil_RecoilValueInterface;
...
var TransactionInterfaceImpl = class {
	constructor(store, treeState) {
		...
		_defineProperty(this, "get", (recoilValue) => {
			if (this._changes.has(recoilValue.key)) return this._changes.get(recoilValue.key);
...
var Recoil_AtomicUpdates_1 = { atomicUpdater }.atomicUpdater;
...
var { atomicUpdater: atomicUpdater$1 } = Recoil_AtomicUpdates$1;
var { batchUpdates: batchUpdates$4 } = Recoil_Batching;
var { DEFAULT_VALUE: DEFAULT_VALUE$5 } = Recoil_Node;
var { useStoreRef: useStoreRef$6 } = Recoil_RecoilRoot;
var { refreshRecoilValue: refreshRecoilValue$1, setRecoilValue: setRecoilValue$3 } = Recoil_RecoilValueInterface;
var { cloneSnapshot: cloneSnapshot$2 } = Recoil_Snapshot$1;
var { gotoSnapshot: gotoSnapshot$1 } = Recoil_SnapshotHooks;
var { useCallback: useCallback$3 } = import_react.default;
var Sentinel = class {};
...
function recoilCallback(store, fn, args, extraInterface) {
	...
	batchUpdates$4(() => {
		...
		const callback = fn(Recoil_lazyProxy({
	...
	ret instanceof Sentinel && Recoil_invariant(false, "batchUpdates should return immediately");
...
var { useStoreRef: useStoreRef$7 } = Recoil_RecoilRoot;
var { refreshRecoilValue: refreshRecoilValue$2 } = Recoil_RecoilValueInterface;
var { useCallback: useCallback$4 } = import_react.default;
...
var { atomicUpdater: atomicUpdater$2 } = Recoil_AtomicUpdates$1;
var { useStoreRef: useStoreRef$8 } = Recoil_RecoilRoot;
var { useMemo: useMemo$3 } = import_react.default;
function useRecoilTransaction(fn, deps) {
	const storeRef = useStoreRef$8();
	return useMemo$3(() => (...args) => {
		atomicUpdater$2(storeRef.current)((transactionInterface) => {
...
var Recoil_Wrapper_1 = { WrappedValue }.WrappedValue;
var Recoil_Wrapper$1 = /* @__PURE__ */ Object.freeze({
	__proto__: null,
	WrappedValue: Recoil_Wrapper_1
});
var { isFastRefreshEnabled: isFastRefreshEnabled$2 } = Recoil_ReactMode;
...
var TreeCache = class {
	...
	set(route, value, handlers) {
		const addLeaf = () => {
			...
			for (const [nodeKey, nodeValue] of route) {
				...
				handlers === null || handlers === void 0 || (_handlers$onNodeVisit = handlers.onNodeVisit) == null || _handlers$onNodeVisit.call(handlers, node);
			...
			handlers === null || handlers === void 0 || (_handlers$onNodeVisit2 = handlers.onNodeVisit) == null || _handlers$onNodeVisit2.call(handlers, leafNode);
		};
		try {
			addLeaf();
		} catch (error) {
	...
	delete(leaf) {
		...
		for (; node !== root; node = node.parent) if (node == null) return false;
		this._numLeafs--;
		return true;
	}
	clear() {
	...
	invalidCacheError() {
		Recoil_recoverableViolation((isFastRefreshEnabled$2() ? "Possible Fast Refresh module reload detected.  This may also be caused by an selector returning inconsistent values. Resetting cache." : "Invalid cache values.  This happens when selectors do not return consistent values for the same input dependency values.  That may also be caused when using Fast Refresh to change a selector implementation.  Resetting cache.") + (this._name != null ? ` - ${this._name}` : ""));
		throw new ChangedPathError();
	}
};
var Recoil_TreeCache_1 = { TreeCache }.TreeCache;
...
var LRUCache = class {
	...
	set(key, val) {
		const mappedKey = this._keyMapper(key);
		if (this._map.get(mappedKey)) this.delete(key);
...
var Recoil_LRUCache_1 = { LRUCache }.LRUCache;
...
function treeCacheLRU({ name, maxSize, mapNodeValue = (v) => v }) {
	const lruCache = new LRUCache$1({ maxSize });
	const cache = new TreeCache$1({
		...
		onSet: (node) => {
			...
			lruCache.set(node, true);
			if (lruNode && cache.size() > maxSize) cache.delete(lruNode.key);
		}
	});
	return cache;
}
...
function stringify(x, opt, key) {
	...
	if (Array.isArray(x)) return `[${x.map((v, i) => stringify(v, opt, i.toString()))}]`;
	if (typeof x.toJSON === "function") return stringify(x.toJSON(key), opt, key);
	if (x instanceof Map) {
		const obj = {};
		for (const [k, v] of x) obj[typeof k === "string" ? k : stringify(k, opt)] = v;
		return stringify(obj, opt, key);
	}
	if (x instanceof Set) return stringify(Array.from(x).sort((a, b) => stringify(a, opt).localeCompare(stringify(b, opt))), opt, key);
	if (Symbol !== void 0 && x[Symbol.iterator] != null && typeof x[Symbol.iterator] === "function") return stringify(Array.from(x), opt, key);
...
function treeCacheFromPolicy({ equality = defaultPolicy.equality, eviction = defaultPolicy.eviction, maxSize = defaultPolicy.maxSize } = defaultPolicy, name) {
	return getTreeCache(eviction, maxSize, getValueMapper(equality), name);
}
function getValueMapper(equality) {
	switch (equality) {
		case "reference": return (val) => val;
		case "value": return (val) => Recoil_stableStringify(val);
	}
	throw Recoil_err(`Unrecognized equality policy ${equality}`);
}
function getTreeCache(eviction, maxSize, mapNodeValue, name) {
	switch (eviction) {
		case "keep-all": return new TreeCache$2({
			name,
			mapNodeValue
		});
...
function isNode(object) {
	...
	const defaultView = (_doc$defaultView = (object != null ? (_ownerDocument = object.ownerDocument) !== null && _ownerDocument !== void 0 ? _ownerDocument : object : document).defaultView) !== null && _doc$defaultView !== void 0 ? _doc$defaultView : window;
	return !!(object != null && (typeof defaultView.Node === "function" ? object instanceof defaultView.Node : typeof object === "object" && typeof object.nodeType === "number" && typeof object.nodeName === "string"));
}
var Recoil_isNode = isNode;
var { isReactNative: isReactNative$1, isWindow: isWindow$1 } = Recoil_Environment;
...
var Recoil_PerformanceTimings = { startPerfBlock };
var { isLoadable: isLoadable$1, loadableWithError: loadableWithError$1, loadableWithPromise: loadableWithPromise$1, loadableWithValue: loadableWithValue$2 } = Recoil_Loadable$1;
var { WrappedValue: WrappedValue$1 } = Recoil_Wrapper$1;
var { getNodeLoadable: getNodeLoadable$2, peekNodeLoadable: peekNodeLoadable$1, setNodeValue: setNodeValue$3 } = Recoil_FunctionalCore;
var { saveDepsToStore: saveDepsToStore$1 } = Recoil_Graph;
var { DEFAULT_VALUE: DEFAULT_VALUE$6, getConfigDeletionHandler: getConfigDeletionHandler$1, getNode: getNode$6, registerNode: registerNode$1 } = Recoil_Node;
var { isRecoilValue: isRecoilValue$3 } = Recoil_RecoilValue$1;
var { markRecoilValueModified: markRecoilValueModified$1 } = Recoil_RecoilValueInterface;
var { retainedByOptionWithDefault: retainedByOptionWithDefault$1 } = Recoil_Retention;
var { recoilCallback: recoilCallback$1 } = Recoil_useRecoilCallback;
var { startPerfBlock: startPerfBlock$1 } = Recoil_PerformanceTimings;
var Canceled = class {};
...
function selector(options) {
	...
	if (typeof key !== "string") throw Recoil_err("A key option with a unique string value must be provided when creating a selector.");
	if (typeof get !== "function") throw Recoil_err("Selectors must specify a get callback option to get the selector value.");
	...
	*/
	function notifyStoresOfNewAsyncDep(store, executionID) {
		if (isLatestExecution(store, executionID)) {
			Recoil_nullthrows(getExecutionInfo(store)).stateVersions.clear();
	...
	function markStoreWaitingForResolvedAsync(store, executionID) {
		...
		if (stores == null) waitingStores.set(executionID, stores = /* @__PURE__ */ new Set());
		stores.add(store);
	}
	/**
	...
	*/
	function wrapResultPromise(store, promise, state, depValues, executionID, loadingDepsState) {
		return promise.then((value) => {
			...
			resolveAsync(store, state, executionID, loadableWithValue$2(value), depValues);
			return value;
			...
			resolveAsync(store, state, executionID, loadableWithError$1(errorOrPromise), depValues);
 ...
 /**
			...
			resolveAsync(store, state, executionID, loadableWithError$1(error), existingDeps);
			throw error;
		});
	}
		...
		*/
		...
		function getRecoilValue({ key: depKey }) {
			...
			switch (depLoadable.state) {
				case "hasValue": return depLoadable.contents;
				case "hasError": throw depLoadable.contents;
				case "loading":
		...
		const getCallback = (fn) => {
			return (...args) => {
				...
				!(recoilValue != null) && Recoil_invariant(false, "Recoil Value can never be null");
				return recoilCallback$1(store, fn, args, { node: recoilValue });
			};
		};
		try {
			result = get({
	...
	function getLoadableFromCacheAndUpdateDeps(store, state) {
		...
		try {
			cachedLoadable = cache.get((nodeKey) => {
				!(typeof nodeKey === "string") && Recoil_invariant(false, "Cache nodeKey is type string");
	...
	*/
	function getInProgressExecutionInfo(store, state) {
		...
		function anyDepChanged(execDepValues) {
			for (const [depKey, execLoadable] of execDepValues) if (!getNodeLoadable$2(store, state, depKey).is(execLoadable)) return true;
			return false;
		}
		for (const execInfo of pendingExecutions) if (execInfo.stateVersions.get(state.version) || !anyDepChanged(execInfo.depValuesDiscoveredSoFarDuringAsyncWork)) {
	...
	*/
	...
	function setCache(state, loadable, depValues) {
		if (loadable.state !== "loading" && Boolean(options.dangerouslyAllowMutability) === false) Recoil_deepFreezeValue(loadable.contents);
	...
	function detectCircularDependencies(fn) {
		if (dependencyStack.includes(key)) return loadableWithError$1(Recoil_err(`Recoil selector has circular dependencies: ${dependencyStack.slice(dependencyStack.indexOf(key)).join(" → ")}`));
		...
		*/
		const selectorSet = (store, state, newValue) => {
			...
			function setRecoilState(recoilState, valueOrUpdater) {
				...
				setNodeValue$3(store, state, recoilState.key, setValue).forEach((v, k) => writes.set(k, v));
			}
			...
			if (ret !== void 0) throw Recoil_isPromise(ret) ? Recoil_err("Recoil: Async selector sets are not currently supported.") : Recoil_err("Recoil: selector set should be a void function.");
...
var { isLoadable: isLoadable$2, loadableWithError: loadableWithError$2, loadableWithPromise: loadableWithPromise$2, loadableWithValue: loadableWithValue$3 } = Recoil_Loadable$1;
var { WrappedValue: WrappedValue$2 } = Recoil_Wrapper$1;
var { peekNodeInfo: peekNodeInfo$3 } = Recoil_FunctionalCore;
var { DEFAULT_VALUE: DEFAULT_VALUE$7, DefaultValue: DefaultValue$2, getConfigDeletionHandler: getConfigDeletionHandler$2, registerNode: registerNode$2, setConfigDeletionHandler: setConfigDeletionHandler$1 } = Recoil_Node;
var { isRecoilValue: isRecoilValue$4 } = Recoil_RecoilValue$1;
var { getRecoilValueAsLoadable: getRecoilValueAsLoadable$4, markRecoilValueModified: markRecoilValueModified$2, setRecoilValue: setRecoilValue$4, setRecoilValueLoadable: setRecoilValueLoadable$2 } = Recoil_RecoilValueInterface;
var { retainedByOptionWithDefault: retainedByOptionWithDefault$2 } = Recoil_Retention;
...
function baseAtom(options) {
	...
	function maybeFreezeValueOrPromise(valueOrPromise) {
		if (options.dangerouslyAllowMutability !== true) if (Recoil_isPromise(valueOrPromise)) return valueOrPromise.then((value) => {
	...
	function wrapPendingPromise(store, promise) {
		const wrappedPromise = promise.then((value) => {
			...
			if (((_state$atomValues$get = ((_store$getState$nextT = store.getState().nextTree) !== null && _store$getState$nextT !== void 0 ? _store$getState$nextT : store.getState().currentTree).atomValues.get(key)) === null || _state$atomValues$get === void 0 ? void 0 : _state$atomValues$get.contents) === wrappedPromise) setRecoilValue$4(store, node, value);
			return value;
		}).catch((error) => {
			var _store$getState$nextT2, _state$atomValues$get2;
			if (((_state$atomValues$get2 = ((_store$getState$nextT2 = store.getState().nextTree) !== null && _store$getState$nextT2 !== void 0 ? _store$getState$nextT2 : store.getState().currentTree).atomValues.get(key)) === null || _state$atomValues$get2 === void 0 ? void 0 : _state$atomValues$get2.contents) === wrappedPromise) setRecoilValueLoadable$2(store, node, loadableWithError$2(error));
			throw error;
		});
	...
	function initAtom(store, initState, trigger) {
		...
		if (defaultLoadable.state === "loading") {
			const notifyDefaultSubscribers = () => {
				...
				if (!((_store$getState$nextT3 = store.getState().nextTree) !== null && _store$getState$nextT3 !== void 0 ? _store$getState$nextT3 : store.getState().currentTree).atomValues.has(key)) markRecoilValueModified$2(store, node);
			};
			...
			const setSelf = (effect) => (valueOrUpdater) => {
				if (isDuringInit) {
					...
					setRecoilValue$4(store, node, typeof valueOrUpdater === "function" ? (currentValue) => {
						const newValue = unwrap(valueOrUpdater(currentValue));
						pendingSetSelf = {
							effect,
							value: newValue
						};
	...
	function getAtom(_store, state) {
		if (state.atomValues.has(key)) return Recoil_nullthrows(state.atomValues.get(key));
			...
			cachedAnswerForUnvalidatedValue = validatorResult instanceof DefaultValue$2 ? defaultLoadable : loadableWithValue$3(validatorResult);
	...
	function setAtom(_store, state, newValue) {
		if (state.atomValues.has(key)) {
			...
			if (existing.state === "hasValue" && newValue === existing.contents) return /* @__PURE__ */ new Map();
		} else if (!state.nonvalidatedAtoms.has(key) && newValue instanceof DefaultValue$2) return /* @__PURE__ */ new Map();
...
function atom(options) {
	if (typeof options.key !== "string") throw Recoil_err("A key option with a unique string value must be provided when creating an atom.");
	const { ...restOptions } = options;
	const optionsDefault = "default" in options ? options.default : new Promise(() => {});
	if (isRecoilValue$4(optionsDefault)) return atomWithFallback({
		...restOptions,
...
var Recoil_MapCache_1 = { MapCache }.MapCache;
var Recoil_MapCache$1 = /* @__PURE__ */ Object.freeze({
	__proto__: null,
	MapCache: Recoil_MapCache_1
});
var { LRUCache: LRUCache$2 } = Recoil_LRUCache$1;
var { MapCache: MapCache$1 } = Recoil_MapCache$1;
var defaultPolicy$1 = {
	equality: "reference",
	eviction: "none",
	maxSize: Infinity
...
function cacheFromPolicy({ equality = defaultPolicy$1.equality, eviction = defaultPolicy$1.eviction, maxSize = defaultPolicy$1.maxSize } = defaultPolicy$1) {
	return getCache(eviction, maxSize, getValueMapper$1(equality));
}
...
function getCache(eviction, maxSize, mapKey) {
	switch (eviction) {
		case "keep-all": return new MapCache$1({ mapKey });
		case "lru": return new LRUCache$2({
			mapKey,
			maxSize: Recoil_nullthrows(maxSize)
		});
		case "most-recent": return new LRUCache$2({
			mapKey,
			maxSize: 1
		});
	}
...
var { setConfigDeletionHandler: setConfigDeletionHandler$2 } = Recoil_Node;
...
var { setConfigDeletionHandler: setConfigDeletionHandler$3 } = Recoil_Node;
var nextIndex = 0;
function selectorFamily(options) {
	...
	return (params) => {
		...
		const myKey = `${options.key}__selectorFamily/${(_stableStringify = Recoil_stableStringify(params, { allowFunctions: true })) !== null && _stableStringify !== void 0 ? _stableStringify : "void"}/${nextIndex++}`;
...
var { loadableWithError: loadableWithError$3, loadableWithPromise: loadableWithPromise$3, loadableWithValue: loadableWithValue$4 } = Recoil_Loadable$1;
...
function wrapLoadables(dependencies, results, exceptions) {
	return wrapResults(dependencies, exceptions.map((exception, idx) => exception == null ? loadableWithValue$4(results[idx]) : Recoil_isPromise(exception) ? loadableWithPromise$3(exception) : loadableWithError$3(exception)));
}
...
var Recoil_WaitFor = {
	waitForNone: Recoil_selectorFamily({
		...
		get: (dependencies) => ({ get }) => {
			const [results, exceptions] = concurrentRequests(get, unwrapDependencies(dependencies));
	...
	waitForAllSettled: Recoil_selectorFamily({
...
var { RecoilLoadable } = Recoil_Loadable$1;
var { DefaultValue: DefaultValue$3 } = Recoil_Node;
var { RecoilRoot: RecoilRoot$2, useRecoilStoreID: useRecoilStoreID$1 } = Recoil_RecoilRoot;
var { isRecoilValue: isRecoilValue$5 } = Recoil_RecoilValue$1;
var { retentionZone: retentionZone$1 } = Recoil_RetentionZone;
var { freshSnapshot: freshSnapshot$2 } = Recoil_Snapshot$1;
var { useRecoilState: useRecoilState$1, useRecoilState_TRANSITION_SUPPORT_UNSTABLE: useRecoilState_TRANSITION_SUPPORT_UNSTABLE$1, useRecoilStateLoadable: useRecoilStateLoadable$1, useRecoilValue: useRecoilValue$1, useRecoilValue_TRANSITION_SUPPORT_UNSTABLE: useRecoilValue_TRANSITION_SUPPORT_UNSTABLE$1, useRecoilValueLoadable: useRecoilValueLoadable$1, useRecoilValueLoadable_TRANSITION_SUPPORT_UNSTABLE: useRecoilValueLoadable_TRANSITION_SUPPORT_UNSTABLE$1, useResetRecoilState: useResetRecoilState$1, useSetRecoilState: useSetRecoilState$1 } = Recoil_Hooks;
var { useGotoRecoilSnapshot: useGotoRecoilSnapshot$1, useRecoilSnapshot: useRecoilSnapshot$1, useRecoilTransactionObserver: useRecoilTransactionObserver$1 } = Recoil_SnapshotHooks;
var { useRecoilCallback: useRecoilCallback$1 } = Recoil_useRecoilCallback;
var { noWait: noWait$1, waitForAll: waitForAll$1, waitForAllSettled: waitForAllSettled$1, waitForAny: waitForAny$1, waitForNone: waitForNone$1 } = Recoil_WaitFor;
var Recoil_index = {
